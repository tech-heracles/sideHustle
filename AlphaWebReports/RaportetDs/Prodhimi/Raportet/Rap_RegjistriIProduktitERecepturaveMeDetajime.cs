...
{
    public partial class Rap_RegjistriIProduktitERecepturaveMeDetajime : DevExpress.XtraReports.UI.XtraReport
    {
		public Rap_RegjistriIProduktitERecepturaveMeDetajime(){InitializeComponent();}
        
        private decimal sasiaplan, sasiakt, dif = 0;
        public Rap_RegjistriIProduktitERecepturaveMeDetajime(AlphaWebReports.Common.ParametraRaporti param, XtraReport report):
            this(param.IdGjuha, param.Ci, param.IdNdermarrje, param.IdViti, param.IdPerdoruesi, report)
        {

        }
        public Rap_RegjistriIProduktitERecepturaveMeDetajime(int idGjuha,CultureInfo ci, int idNdermarrje, int idViti, int idPerdoruesi, DevExpress.XtraReports.UI.XtraReport raport)
        {
            InitializeComponent();
            EmrateLabelave(ci);
            switch (idGjuha)
            ...
            xrLabel21.Text = raport.Parameters[13].Description;
            parameter14.Value = raport.Parameters[13].Value;
            xrLabel26.Text = raport.Parameters[14].Description;
            parameter15.Value = raport.Parameters[14].Value;
            xrLabel28.Text = raport.Parameters[15].Description;
        ...
        private void sasiaPlanLabel_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (GetCurrentColumnValue("ArtProdhim") != null && GetCurrentColumnValue("ArtProdhim").ToString() == "0")
            {
                sasiaplan +=decimal.Parse( this.GetCurrentColumnValue("SasiPlan").ToString(), NumberStyles.Any);
                sasiakt += decimal.Parse(this.GetCurrentColumnValue("SasiAkt").ToString(), NumberStyles.Any);
                dif += decimal.Parse(this.GetCurrentColumnValue("Diferenca").ToString(), NumberStyles.Any); ;
            }
        }

        private void xrLabel23_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            xrLabel23.Text = String.Format("{0:#.00}", sasiaplan);
        }

        private void Rap_RegjistriIProduktitERecepturaveMeDetajime_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
