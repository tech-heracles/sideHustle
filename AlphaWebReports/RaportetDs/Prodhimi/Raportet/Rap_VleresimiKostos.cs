...
{
    public partial class Rap_VleresimiKostos : DevExpress.XtraReports.UI.XtraReport
    {
		public Rap_VleresimiKostos(){InitializeComponent();}
        
        double vlefta;
        double sasiaakt;
        public Rap_VleresimiKostos(AlphaWebReports.Common.ParametraRaporti param, XtraReport report):
            this(param.IdGjuha, param.Ci, param.IdNdermarrje, param.IdViti, param.IdPerdoruesi, report)
        {

        }
        public Rap_VleresimiKostos(int idGjuha,CultureInfo ci, int idNdermarrje, int idViti, int idPerdoruesi, DevExpress.XtraReports.UI.XtraReport raport)
        {
            InitializeComponent();
            EmrateLabelave(ci);
            switch (idGjuha)
        ...
        private void Detail_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (GetCurrentColumnValue("ArtProdhim") != null && GetCurrentColumnValue("ArtProdhim").ToString() == "1")
            {
              //  llojiLabel.Font = new System.Drawing.Font("Times New Roman", 10F, System.Drawing.FontStyle.Bold);
                ...
                sasiaakt = Convert.ToDouble(GetCurrentColumnValue("SasiAkt"));
                sasiaPlanLabel.Visible = false;
                diferencaLabel.Visible = false;
                cmimiLabel.Visible = false;
                vleftaLabel.Visible = false;
            }
            else
            {
                sasiaPlanLabel.Visible = true;
                diferencaLabel.Visible = true;
                cmimiLabel.Visible = true;
                vleftaLabel.Visible = true;
                //kur sasia aktuale eshte negative mbetja eshte 0
                if (GetCurrentColumnValue("SasiAkt") != null) {
                    double sasiaakt = Convert.ToDouble(GetCurrentColumnValue("SasiAkt"));
                    if (sasiaakt<0)
                        diferencaLabel.Text = String.Format("{0:#,#.00}", 0);
                    else
                        if (GetCurrentColumnValue("Diferenca")!=null)
        ...
        private void EmrateLabelave(CultureInfo ci)
        {
            ...
            xrLabel12.Text = rm.GetString("lblRaportVleresimKosto", ci);
            ...
            xrLabel19.Text = rm.GetString("labelRaportMbetje", ci);
            xrLabel35.Text = rm.GetString("labelLogoIMB", ci);
           
        }

        private void vleftaLabel_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if ((this.GetCurrentColumnValue("Vlefta")!=null) && (vleftaLabel.Visible==true))
            vlefta += Convert.ToDouble(this.GetCurrentColumnValue("Vlefta"));
        }

        private void xrLabel20_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {

            if ((this.GetCurrentColumnValue("Vlefta")!=null)&&(sasiaakt!=0))
            xrLabel20.Text = String.Format("{0:#,#.00}", (vlefta / sasiaakt));
            else xrLabel20.Text = String.Format("{0:#,#.00}", 0);
        }
        ...
        {
            xrLabel5.Text = String.Format("{0:#,#.00}",vlefta);
        }

        private void Rap_VleresimiKostos_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
