...
function hapraportin(s) {
    var width = $(window).width();
    myFaqeCelje.kontrolloTeDrejta("Raporti.aspx?idraporti=" + s + "&windowWidth=" + width + "&radButon=" + btnradPeriudha.GetSelectedItem().value + "&dateNga=" + dtdoknga.GetText() + "&dateDeri=" + dtdokderi.GetText() + "&Filtro=false");

    var ngaCRM = Utils.getUrlVar("vjenNga");
   window.location="Raporti.aspx?idraporti=" + s + "&windowWidth=" + width + "&radButon=" + btnradPeriudha.GetSelectedItem().value + "&dateNga=" + dtdoknga.GetText() + "&dateDeri=" + dtdokderi.GetText() + "&Filtro=false&vjenNga="+ngaCRM;
}


function filtroButtonClick(idRaporti) {
    var width = $(window).width();
    //myFaqeCelje.kontrolloTeDrejta("Raporti.aspx?idraporti=" + idRaporti + "&windowWidth=" + width + "&radButon=&dateNga=&dateDeri=&Filtro=true");
    var ngaCRM =

    window.location = "Raporti.aspx?idraporti=" + idRaporti + "&windowWidth=" + width + "&radButon=&dateNga=&dateDeri=&Filtro=true&vjenNga="+ngaCRM;
}
...
function expandAll(s, e) {
    ...
    var splitpane4 = splitpane3.GetPane(0);
    var splitpane5 = splitpane3.GetPane(1);
    splitpane0.Collapse(splitpane1);
    splitpane2.Collapse(splitpane3);
    splitpane5.Collapse(splitpane4);
}
