...
{
    public partial class Rap_ShitjeAnalitik_Barkod : DevExpress.XtraReports.UI.XtraReport
    {
		public Rap_ShitjeAnalitik_Barkod(){InitializeComponent();} 

        int shifraPasPresjes = 0;
        public Rap_ShitjeAnalitik_Barkod(AlphaWebReports.Common.ParametraRaporti param, XtraReport report):
            this(param.Ci, param.IdNdermarrje, param.IdViti, param.IdPerdoruesi, report)
        {

        }
        public Rap_ShitjeAnalitik_Barkod(CultureInfo ci, int idNdermarrje, int idViti, int idPerdoruesi, DevExpress.XtraReports.UI.XtraReport raport)
        {
            InitializeComponent();
            EmrateLabelave(ci);
        ...
        private void EmrateLabelave(CultureInfo ci)
        {
          ResourceManager  rm = new ResourceManager("Resources.Strings",
            ...
            DokumentArtikulliR.Text = rm.GetString("labelDokumentArtikulli", ci);
            KodiR.Text = rm.GetString("labelKodi", ci);
            PershkrimiR.Text = rm.GetString("labelRaportiPershkrimi", ci);
            NjesiaR.Text =  rm.GetString("labelNjesia", ci);
            SasiaR.Text =  rm.GetString("labelSasia", ci);
            VleftaR.Text = rm.GetString("labelVlefta", ci);
            GjithsejR.Text =  rm.GetString("labelGjithsej", ci);
            ZbritjaAnalitikeR.Text =  rm.GetString("labelZbritjeAnalitike", ci) + " %";
            ZbritjaTotaleR.Text = rm.GetString("labelZbritjaTotale", ci) + " %";
            VleftameZbritjeR.Text =rm.GetString("labelVleftameZbritje", ci);
            MonLlogariR.Text =rm.GetString("labelMonLlogari", ci);
            MonBazeR.Text =rm.GetString("labelMonBaze", ci);
            PerdoruesiR.Text =rm.GetString("labelFilterAvancuarPerdorues", ci);
            PershkrimFatureR.Text =rm.GetString("filterRaportPershkrimFature", ci);
        }
