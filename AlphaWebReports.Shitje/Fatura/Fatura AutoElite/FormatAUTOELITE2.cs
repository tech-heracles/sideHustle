...
{  
    public partial class FormatAUTOELITE2 : DevExpress.XtraReports.UI.XtraReport
    {
		public FormatAUTOELITE2(){InitializeComponent();} 
        public FormatAUTOELITE2(AlphaWebReports.Common.ParametraRaporti param, XtraReport report):
            this(param.Ci, param.IdNdermarrje, param.IdPerdoruesi)
        {

        }
        public FormatAUTOELITE2(CultureInfo ci, int idNdermarrje, int idPerdoruesi)
        {
            InitializeComponent();
            EmrateLabelave(ci);
        }
        ...
        private void EmrateLabelave(CultureInfo ci)
        {
            ...
            xrLabel1.Text = rm.GetString("RaportLibretAutomjetiTitulli", ci);

            xrLabel50.Text = rm.GetString("labelSubjektiUpperCase", ci);
            xrLabel27.Text = rm.GetString("labelAdresaUpperCase", ci);
            xrLabel29.Text = rm.GetString("labelTelUpperCase", ci);
            ...
            xrTableCell10.Text = rm.GetString("labelTotalNeto", ci);

            xrLabel12.Text = rm.GetString("labelShitesiUpperCase", ci) + ":";

            xrLabel14.Text = rm.GetString("labelTotaliUpperCase", ci);
            xrLabel23.Text = rm.GetString("labelTotaliNe", ci);
            xrLabel25.Text = rm.GetString("labelKursiUpperCase", ci);


            xrLabel6.Text = rm.GetString("labelDetyrimParaUpperCase", ci);
            xrLabel32.Text = rm.GetString("labelPageseUpperCase", ci);
            xrLabel34.Text = rm.GetString("labelDetyrimTotalUpperCase", ci);
