...
namespace DbCore.DbRegjistrim
{
    ...
    public sealed class clsKokaShitje
    {
        ...
        public string changeDocNoIfExists(string docNo, DateTime date, int enterpriseId, clsNrAutom nrAuto)
        {
            bool exists = false;
            using (clsDatabaseRegjistrim db = new clsDatabaseRegjistrim())
            {
                exists = db.checkSale(docNo, date, enterpriseId);
            }

            if (exists)
            {

                clsNrAutomatikFundit lastNumber = nrAuto.ktheNrAutomatikFundit(date);
                docNo = nrAuto.gjeneroNumrinAutomatikPasardhes(lastNumber.Vlera);
