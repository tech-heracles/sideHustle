...
{
    public partial class Rap_ListpagesaPermbledhese : DevExpress.XtraReports.UI.XtraReport
    {
		public Rap_ListpagesaPermbledhese(){InitializeComponent();} 
        
   
     
        public Rap_ListpagesaPermbledhese(AlphaWebReports.Common.ParametraRaporti param, XtraReport report):
            this(param.Ci, param.IdNdermarrje, param.IdViti, param.IdPerdoruesi, param.ScopeID, report)
        {

        }
        public Rap_ListpagesaPermbledhese(CultureInfo ci, int idNdermarrje, int idViti, int idPerdoruesi, string scopeID, XtraReport report)
        {
            InitializeComponent();
            EmrateLabelave(ci);
