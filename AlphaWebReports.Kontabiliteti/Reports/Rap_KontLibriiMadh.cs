...
namespace AlphaWebReports.RaportetDs
    {
    public partial class Rap_KontLibriiMadh : DevExpress.XtraReports.UI.XtraReport
        {
		public Rap_KontLibriiMadh(){InitializeComponent();} 
        ...
        public Rap_KontLibriiMadh(ParametraRaporti param, XtraReport report) :
            this(param.Ci, param.IdNdermarrje, param.IdViti, param.IdPerdoruesi,param.ScopeID, report)
        {

        }
        public Rap_KontLibriiMadh(CultureInfo ci, int idNdermarrje, int idViti, int idPerdoruesi,string scopeID, DevExpress.XtraReports.UI.XtraReport raport)
            {
            InitializeComponent();
        ...
        private void xrLabel37_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            {
                xrLabel37.Text = String.Format("{0:#,#.00}", vleraProgresive);
                vleraProgresive = 0;
            }
            else xrLabel37.Text = String.Format("{0:#,#.00}", 0);
        ...
        private void xrLabel39_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
            {
                if (vleraProgresive < 0)
                {
                    xrLabel39.Text = String.Format("{0:#,#.00}", Math.Abs(vleraProgresive));
                    vleraProgresive = 0;
                }
        ...
        private void xrLabel40_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
            ...
            if (vleraProgresive2 > 0)
                {
                    xrLabel40.Text = String.Format("{0:#,#.00}", vleraProgresive2);
                    vleraProgresive2 = 0;
                }
        ...
        private void xrLabel41_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
            {
                if (vleraProgresive2 < 0)
                {
                    xrLabel41.Text = String.Format("{0:#,#.00}", Math.Abs(vleraProgresive2));
                    vleraProgresive2 = 0;
                }
            else  xrLabel41.Text = String.Format("{0:#,#.00}", 0);
        }
        ...
        private void xrLabel68_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
            {
            double sum = 0;
            for (int i = 0; i < e.CalculatedValues.Count; i++)
                sum += Convert.ToDouble(e.CalculatedValues[i]);
        ...
        private void xrLabel1_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
            {
                ...
                if (GetCurrentColumnValue("IDKOKAFLETEKONTABEL") != null && GetCurrentColumnValue("NRKOKAFLETEKONTABEL") != null)
                {
                ...
                xrLabel1.NavigateUrl = "javascript:window.myFaqeCelje.hapFaqeNeTabTeRi('Shto_FleteKontabel.aspx?scopeID=" + parametraRaporti.ScopeID + "&newScopeId=True&shtim_modifikim=modifikim&id=" + GetCurrentColumnValue("IDKOKAFLETEKONTABEL").ToString() + "&numur=" + GetCurrentColumnValue("NRKOKAFLETEKONTABEL").ToString() + "')";
        ...
        private void EmrateLabelave(CultureInfo ci)
        {
            ...
            xrLabel17.Text = rm.GetString("RaportLibriMadhTitulli", ci);
            ...
            xrLabel23.Text = rm.GetString("labelRaportiProgresivi", ci);
            xrLabel19.Text = rm.GetString("labelRaportiDebi", ci);
            xrLabel16.Text = rm.GetString("labelRaportiKredi", ci);
            xrLabel24.Text = rm.GetString("labelRaportiProgresivi", ci);
            xrLabel36.Text = rm.GetString("labelRaportiGjendjaMePare", ci);
            xrLabel28.Text = rm.GetString("labelRaportiTotali", ci);
            xrLabel20.Text = rm.GetString("labelFilterAvancuarGjendja", ci);
            xrLabel47.Text = rm.GetString("labelRaportiTotaliGjithsej", ci);
            xrLabel65.Text = rm.GetString("labelRaportiGjendjaGjithsej", ci);
            xrLabel27.Text = rm.GetString("labelLogoIMB", ci);
        }

        private void Rap_KontLibriiMadh_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
