...
function Init() {
    ...
    try {
        ...
        gvLupaTipKontrate.SetFocusedRowIndex(0);
        btnOk.Focus();
    }
    catch (err) {
        ///alert('gabim');    
...
function ProcessKeyPress() {
    var currentIndex = gvLupaTipKontrate.GetFocusedRowIndex();
    if (event.keyCode === 40) {
        if (currentIndex === gvLupaTipKontrate.GetVisibleRowsOnPage() - 1) {
            gvLupaTipKontrate.SetFocusedRowIndex(0);
        }
        else {
            gvLupaTipKontrate.SetFocusedRowIndex(currentIndex + 1);
        }
    }
    if (event.keyCode === 38) {
        if (currentIndex === 0) {
...
function OnGridSelectionChanged() {
    gvLupaTipKontrate.GetRowValues(gvLupaTipKontrate.GetFocusedRowIndex(), 'IdTipKontrate;Kodi;KodiAng', OnGridSelectionComplete);
}
// kur bejme double klick qe kalojme te dhenat tek faqja kryesore
function OnGridSelectionComplete(values) {
    if($('#hfGjuha').val()==0)
        window.parent.cmbTipKontrate.SetSelectedIndex(window.parent.cmbTipKontrate.AddItem(values[1], values[0]));
    else 
    window.parent.cmbTipKontrate.SetSelectedIndex(window.parent.cmbTipKontrate.AddItem(values[2], values[0]));
    window.parent.cmbTipKontrate.SetFocus(true);
...
function switchEditMode(index) {//kalon ne edit grida
    ...
    if (editmode) {
        if (index === indexEdit) {
            ...
            gvLupaTipKontrate.StartEditRow(index);
            indexEdit = index;
        }
    }
    else {
...
function Item_Click(s, e) {
    ...
    { gvLupaTipKontrate.UpdateEdit();    e.processOnServer=false;
        modifiko=false;
    }   
