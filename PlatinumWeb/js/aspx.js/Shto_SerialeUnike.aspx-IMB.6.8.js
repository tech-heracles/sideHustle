...
function aplikoFiltra(s, e) {
    btnFiltrat = s;
    myMenu.aplikoFiltra(s, e, gvSerialet, "446", cmbKonfigurimi.GetText());
}
...
function Poshte_click(e) {
    indexSel = myMenu.JSlevizNeGride.Poshte_click(e, gvSerialet, indexSel);
}
/*Function: Lart_click
...
function Lart_click(e) {
    indexSel = myMenu.JSlevizNeGride.Lart_click(e, gvSerialet, indexSel);
}
/*
...
function Fillim_click(e) {
    indexSel = myMenu.JSlevizNeGride.Fillim_click(e, gvSerialet, indexSel);
}
/*
Function: Fund_click
...
function Fund_click(e) {
    indexSel = myMenu.JSlevizNeGride.Fund_click(e, gvSerialet, indexSel);
}
...
function mbushfusha() {
    ...
    indexModifiko = gvSerialet.GetFocusedRowIndex();
    if (indexModifiko == -1)
        myMesazh.ShtoMesazhGabimi('Duhet te zgjidhni nje serial!');
    else
        gvSerialet.GetRowValues(indexModifiko, 'ID;Kodi;Emertimi;LlojSeriali;NrKaraktere;FormuleSpecifike;Aktiv;SerialKryesor', OnGetRowValuesMod); Utils.shfaqLoadingGif();;
}
...
function OnGetRowValuesMod(values) {
    ...
    cmbKaraktere.SetText(values[4].split(' ')[0]);
    txtNrKaraktere.SetText(values[4].split(' ')[1]);
    txtFormule.SetText(values[5]);
    cbAktiv.SetChecked(values[6]);
    cbKryesor.SetChecked(values[7]);
    ...
    $.ajax({
        ...
        data: JSON.stringify({ idkomp: "446", kodkonfi: cmbKonfigurimi.GetText(), iddokumenti: values[0], idNdermarrje: hfState.Get('idNdermarrje'), idGjuha: hfState.Get('idGjuha') })
...
function pastrofusha() {
    txtKodi.SetText('');
    txtEmertimi.SetText('');
    cmbLloji.SetSelectedIndex(0); cmbKaraktere.SetSelectedIndex(0);
    txtNrKaraktere.SetText('');
    txtFormule.SetText('');
    cbAktiv.SetChecked(true);
    cbKryesor.SetChecked(false);
...
function callWebserviceKonfigurimi(idKomp, kodKonf) {
    ...
    gvSerialet.PerformCallback(idKomp + ";" + kodKonf);
}
...
function ndryshoKonfigurimin() {
    ...
    callWebserviceKonfigurimi("446", cmbKonfigurimi.GetText());
}
function ndryshoKonfiguriminInit() {

    callWebserviceKonfigurimiInit("446", cmbKonfigurimi.GetText());
}
...
function changeName() {
    ...
    myFaqeCelje.changeName('Shto_SerialeUnike.aspx', 0, hf);
...
function EndRequestHandler(sender, args) {
    ...
    indexModifiko = myFaqeCelje.EndRequestHandlerNew(sender, args, hf, colKontrollet, colAtrTrupi, hfShtimModifikim, hfId, indexModifiko, PageControl, gvSerialet, "446", pastrofusha, hfTeDrejta);
}
