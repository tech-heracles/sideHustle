...
$(window).load(function () {
    try {
        gvLupaArtikullPerberes.SetWidth(document.documentElement.clientWidth - 20);   
        Init();
    }
    catch (e) {
    }
...
function Init() {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ idKomp: 917, kodKonf: 'LP/ShfaqArtPerb', idNdermarrje: idNdermarrje, idGjuha: idGjuha })
...
function ProcessKeyPress() {
    var currentIndex = gvLupaArtikullPerberes.GetFocusedRowIndex();
    if (event.keyCode == 40) {
        if (currentIndex == gvLupaArtikullPerberes.GetVisibleRowsOnPage() - 1) {
            gvLupaArtikullPerberes.SetFocusedRowIndex(0);
        }
        else {
            gvLupaArtikullPerberes.SetFocusedRowIndex(currentIndex + 1);
        }
    }
    if (event.keyCode == 38) {
        if (currentIndex == 0) {
...
function Filtra_Click() {
    window.parent.myButtonClickLupa.LupaUniversal_Click(hfState.Get("headerShtoFilter"), 'LupaFiltra.aspx?grida=gvLupaArtikullPerberes&page=LupaArtikull.aspx&idKonfigAmbjente=577', 850, 450);
    popFiltra.Show();
}
...
function MerrGjendjeKosto() {
    ...
    //gvLupaArtikullPerberes.PerformCallback("kerko;610;LP/Art;" + mag);
}
