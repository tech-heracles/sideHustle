...
function ndryshoKonfigurimin() {
    ...
    gvProkurimetParashikim.PerformCallback(3039 + ";" + cmbKonfigurimi.GetText());
}
...
function StartEditing(s, e) {
    ...
    if (hfState.Get("raportuese") || ( e.focusedColumn.fieldName != 'FondiLimit'
        && e.focusedColumn.fieldName != 'LlojProcedure'
        && e.focusedColumn.fieldName != 'KoheTenderi'
        && e.focusedColumn.fieldName != 'BurimiFinancimit'
        )
...
function menu_click(s, e) {
    ...
    if (e.item.name == "Ruaj") {
        gvProkurimetParashikim.UpdateEdit();


    }
    else if (e.item.name == "AnulloNdryshimet") {
        gvProkurimetParashikim.CancelEdit();
        gvProkurimetParashikim.Refresh();
    }
