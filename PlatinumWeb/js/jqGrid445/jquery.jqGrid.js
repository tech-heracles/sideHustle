	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				if (ts.p.gridview === false) {
				    myListRowData.push(rowData.join(''));
					//$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join(''));
					//self.triggerHandler("jqGridAfterInsertRow", [idr, rd, cur]);
				    //if(afterInsRow) {ts.p.afterInsertRow.call(ts,idr,rd,cur);}
					if (afterInsRow) {
					    setTimeout(function (xts, xidr, xrd, xcur,xself) {
					        return function () {
					            xself.triggerHandler("jqGridAfterInsertRow", [xidr, xrd, xcur]);
					            xts.p.afterInsertRow.call(xts, xidr, xrd, xcur);
					        };
					    }(ts, idr, rd, cur,self), 0);
					};
					rowData=[];//ari=0;
				}
				rd={};
			...
			if (ts.p.gridview === false) {
			    $("#" + $.jgrid.jqID(ts.p.id) + " tbody:first").append(myListRowData.join(''));
			}
			if(ts.p.gridview === true ) {
				fpos = ts.p.treeANode > -1 ? ts.p.treeANode: 0;
				if(ts.p.grouping) {
