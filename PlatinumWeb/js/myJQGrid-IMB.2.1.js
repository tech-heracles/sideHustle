...
$.jgrid.extend({
    ...
    ktheSasiPerArtikullinMeDetajim: function (kodArtShtim, fushat) {
        ...
        var rreshtaArt = dataset.filter(function (el) {
            return (el[fushat.kodi].toLowerCase() == kodArtShtim.toString().toLowerCase() || el[fushat.kodbari].toLowerCase() == kodArtShtim.toString().toLowerCase())
        });
    ...
    SetTekst: function (editor, idRow, value) {
        this.selektoRreshtin(idRow);
        this.setFocus(editor, idRow);
        this.setTekstQelize(editor, idRow, value);
    },
    VendosDisaRreshtaMeSubjekte: function (vlerat, rreshtiPare, funcKontrolloSubjekt, funcVendosSubjekt, funcMesazh) {
        var emerGride = this.selector;
        var fushaIdentity = myJQGrid.ktheFushaIdentity(emerGride);
        var rreshtiBosh = rreshtiPare;
        var funcMesazhParam = new Array();
        vlerat.forEach(function(item, i) {
            if (!funcKontrolloSubjekt(item)) {
                if (funcMesazh)
                    funcMesazhParam.push(item);
                return;
            }

            if (i > 0) {
                if (rreshtiBosh == -1) {
                    $(emerGride).shtoRresht(false, emerGride);
                    rreshtiBosh = $(emerGride).gjeRreshtBosh(fushaIdentity.fusheKontrolli);
                }
            }

            funcVendosSubjekt(item, rreshtiBosh);
            rreshtiBosh = $(emerGride).gjeRreshtBosh(fushaIdentity.fusheKontrolli);
        });
        rreshtiBosh = this.gjeRreshtBosh(fushaIdentity.fusheKontrolli);
        if (rreshtiBosh == -1)
            this.shtoRresht(false, emerGride);
        this.rregulloNrRendor();
        if (funcMesazh) {
            funcMesazh(funcMesazhParam);
        }
    }
});
...
myJQGrid.WarnGjendje = function (minArt, maxArt, sasia, magazina, kodart, hyrjeOseBlerje) {
    if (minArt != 0.00 && parseFloat(sasia) < parseFloat(minArt)) {
        if (magazina != '' && magazina !== undefined )
...
myJQGrid.cancelQuickChange = function (params) {//event, ui, emerKodi, changeFunc
    ...
    var timeToWaitForCancel = 400;
    if (params.timeToWait && params.timeToWait != 400)
        timeToWaitForCancel = params.timeToWait;
...
myJQGrid.myElemKodi = function (value, option, disabled, idRreshti, id, buttonClickKodi, keyUpKodi, changefunction, fokusi, lostFocusKoloneFundit, timeToWait) {
    ...
    textField.on("keyup", function (event) {
        if (event.which == 13) { //enter
            ...
            if (id == 'txtDetajimi' || id == 'txtDetajimi2t')
                return false;

            if (changefunction)
                changefunction(event, null, id, idRreshti);
                //myJQGrid.cancelQuickChange({ event: event, ui: null, emerKodi: id, changeFunc: changefunction, idRreshti: idRreshti, timeToWait: 50 }); //changeFunc(event, null, null);
            else
    ...
    textField.on("focusout", function (event) {
        ...
        if (changefunction)
            myJQGrid.cancelQuickChange({ event: event, ui: null, emerKodi: id, changeFunc: changefunction, idRreshti: idRreshti, timeToWait: timeToWait }); //changeFunc(event, null, null);
        else
...
myJQGrid.myElemButtonSerialetUnike = function (disabled, lastsel) {
    return "<input id='txtSerialUnik" + lastsel + "' type='image' value='SU' " + (disabled ? " disabled='disabled' " : " ") + " src='images/serialetUnike1.png' onclick='ButtonSerialeUnikeClicked(" + lastsel + ", event)'/>";
};
...
myJQGrid.myValueButtonSerialeUnike = function (disabled, lastsel) {
    return "<div><input id='bsu" + lastsel + "' type='image' value='SU' " + (disabled ? " disabled='disabled' " : " ") + " src='images/serialetUnike.png' onclick='ButtonSerialeUnikeClicked(" + lastsel + ", event)'/></div>";
};
...
myJQGrid.checkIfIsTheSame = function (grida, artikulli, detajimi, idRreshti, lloji, kodbariArtNeGride, kodbarisele, kerkoMeKodbar) {
    if (grida.getTekstQelize('txtIdKodi', idRreshti) == artikulli.IdArtikulli && lloji == "Artikull") {
        if (typeof (kodbariArtNeGride) !== 'undefined' && kodbariArtNeGride)
...
myJQGrid.beginWebserviceCmimArtikulliRow = function (idRreshti, emerfushe) {
    var counterCmimi = 0;
    if ($("#" + emerfushe + idRreshti).data('pending') && $("#" + emerfushe + idRreshti).data('pending') != "")
        counterCmimi = parseInt($("#" + emerfushe + idRreshti).data('pending'));
    $("#" + emerfushe + idRreshti).data('pending', counterCmimi + 1);
};

myJQGrid.endWebserviceCmimArtikulliRow = function (idRreshti, emerfushe) {
    if ($("#" + emerfushe + idRreshti).data('pending') && $("#" + emerfushe + idRreshti).data('pending') != "") {
        var counterCmimi = $("#" + emerfushe + idRreshti).data('pending');
        if (counterCmimi == 1)
            $("#" + emerfushe + idRreshti).removeData('pending');
        else
            $("#" + emerfushe + idRreshti).data('pending', counterCmimi - 1);
    }
};