...
function dbClickGrida() {
    document.parentWindow.parent.document.getElementById('Container').src = 'RaportiEPaySlip.aspx';
}
function hapraportin(sender, args) {
    if (args.get_error() == undefined) {
        if (sender._postBackSettings.panelID == "pnlKryesor|menuKryesore" && document.getElementById('hdf1').value == '1') {
            ...
            window.open("RaportiEPaySlip.aspx");
        }
    }
    else
...
function Pageload(s, e) {
    ImbReportToolbar1.OnPageLoad(e);
    if (faqjere) {
        var windowWidth = $(window).width();
        var idRaporti = Utils.getUrlVar("idraporti");
        window.open("RaportiShpejte.aspx?idraporti=" + idRaporti + "&printo=0&Sesioni=true&windowWidth=" + windowWidth + "&Orientimi=" + ImbReportToolbar1.GetOrientation() + "&reportStyle=" + ImbReportToolbar1.GetStyle() + "&scopeID=" + Utils.getUrlVar("scopeID"));
        faqjere = false;
    }
}
...
function menu_click(s, e) {
    ...
    if (e.item.name == 'Pastro') {
        ...
        $('#hfRilodo').val(true);
        ImbReportToolbar1.ResetArsyeReload();
