...
{
    public partial class RAP_ARKA_DITARITOTAL_Gethe : DevExpress.XtraReports.UI.XtraReport
    {
		public RAP_ARKA_DITARITOTAL_Gethe(){InitializeComponent();} 
     
        private CultureInfo ci;
        private ResourceManager rm = new ResourceManager("Resources.Strings", System.Reflection.Assembly.Load("App_GlobalResources"));

        public RAP_ARKA_DITARITOTAL_Gethe(AlphaWebReports.Common.ParametraRaporti param, XtraReport report):
            this(param.Ci, param.IdNdermarrje, param.IdViti, param.IdPerdoruesi, report)
        {

        }

        public RAP_ARKA_DITARITOTAL_Gethe(CultureInfo ci, int idNdermarrje, int idViti, int idPerdoruesi, DevExpress.XtraReports.UI.XtraReport raport)
        {
            this.ci = ci;
            InitializeComponent();
            EmrateLabelave(ci);
            ...
            parameterFAA.Value = rm.GetString("cmbboxItemFilterAvancArka", ci).ToString();
            parameterFAB.Value = rm.GetString("cmbboxItemFilterAvancBanka", ci).ToString();
        }
        ...
        private void EmrateLabelave(CultureInfo ci)
        {
            ...
            xrTableCell4.Text = rm.GetString("labelRaportiEmertimi", ci);
